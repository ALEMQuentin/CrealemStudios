...
class Kernel
{
    ...
    private function handleMedia(string $action): void
    {
        ...
        if ($action === 'create') {
            ...
            $isEdit = false;
            $this->render('Ajouter un média', $this->resolveView(['modules/media-form.php']), compact('mediaItem', 'isEdit'));
            return;
        }
        ...
        if ($action === 'edit') {
            ...
            $isEdit = true;
            $this->render('Modifier un média', $this->resolveView(['modules/media-form.php']), compact('mediaItem', 'isEdit'));
            return;
        }
        ...
        if ($action === 'save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($id > 0) {
                $existing = $this->fetchOne("SELECT * FROM media WHERE id = :id", ['id' => $id]);
                if (!$existing) {
                    redirectTo('/admin.php?module=media&error=Média introuvable');
                }

                $this->updateRow('media', $id, [
                    'title' => trim($_POST['title'] ?? ''),
                    'alt_text' => trim($_POST['alt_text'] ?? ''),
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);

                redirectTo('/admin.php?module=media&success=Média enregistré');
            }

            if (
                !isset($_FILES['media_file']) ||
                !is_array($_FILES['media_file']) ||
                (int)($_FILES['media_file']['error'] ?? 1) !== 0
            ) {
                redirectTo('/admin.php?module=media&error=Fichier média manquant');
            }

            $originalName = (string)($_FILES['media_file']['name'] ?? 'file');
            $tmpName = (string)($_FILES['media_file']['tmp_name'] ?? '');
            $mimeType = (string)($_FILES['media_file']['type'] ?? '');
            $safeName = preg_replace('/[^A-Za-z0-9._-]+/', '-', $originalName);
            $finalName = time() . '-' . $safeName;

            $projectRoot = dirname(__DIR__, 2);
            $targetDir = $projectRoot . '/public/uploads';
            if (!is_dir($targetDir)) {
            ...
            if (!move_uploaded_file($tmpName, $targetPath)) {
                redirectTo('/admin.php?module=media&error=Impossible de déplacer le fichier');
            }
        ...
        if ($action === 'delete') {
            ...
            $projectRoot = dirname(__DIR__, 2);
            $filePath = $projectRoot . '/public' . ($mediaItem['filepath'] ?? '');
            if (is_file($filePath)) {
                @unlink($filePath);
            }

...
<form method="post" action="/admin.php?module=products&action=save<?= !empty($isEdit) ? '&id=' . (int)$product['id'] : '' ?>">
    <div class="row mt-4">
        <div class="col-md-8">
            ...
            <div class="card mt-4">
                <div class="card-body">
                    <h3 class="h6 mb-3">Données produit</h3>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mt-4">
                            <label class="form-label">Gérer le stock</label>
                            <select class="form-select" name="manage_stock">
                                <option value="0" <?= ((int)($product['manage_stock'] ?? 0) === 0) ? 'selected' : '' ?>>Non</option>
                                <option value="1" <?= ((int)($product['manage_stock'] ?? 0) === 1) ? 'selected' : '' ?>>Oui</option>
                            </select>
                        </div>

                        <div class="col-md-3 mt-4">
                            <label class="form-label">Quantité</label>
                            <input type="number" class="form-control" name="stock_quantity" value="<?= e((string)($product['stock_quantity'] ?? '')) ?>">
                        </div>

                        <div class="col-md-3 mt-4">
                            <label class="form-label">État du stock</label>
                            <select class="form-select" name="stock_status">
                                <option value="instock" <?= (($product['stock_status'] ?? 'instock') === 'instock') ? 'selected' : '' ?>>En stock</option>
                                <option value="outofstock" <?= (($product['stock_status'] ?? '') === 'outofstock') ? 'selected' : '' ?>>Rupture</option>
                                <option value="onbackorder" <?= (($product['stock_status'] ?? '') === 'onbackorder') ? 'selected' : '' ?>>Commande en attente</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-3 mt-4">
                            ...
                            <input type="number" step="0.01" class="form-control" name="weight" value="<?= e((string)($product['weight'] ?? '')) ?>">
                        </div>

                        <div class="col-md-3 mt-4">
                            <label class="form-label">Longueur</label>
                            <input type="number" step="0.01" class="form-control" name="length" value="<?= e((string)($product['length'] ?? '')) ?>">
                        </div>

                        <div class="col-md-3 mt-4">
                            <label class="form-label">Largeur</label>
                            <input type="number" step="0.01" class="form-control" name="width" value="<?= e((string)($product['width'] ?? '')) ?>">
                        </div>

                        <div class="col-md-3 mt-4">
                            <label class="form-label">Hauteur</label>
                            <input type="number" step="0.01" class="form-control" name="height" value="<?= e((string)($product['height'] ?? '')) ?>">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card mt-4">
                <div class="card-body">
                    <h3 class="h6 mb-3">Attributs du produit</h3>

                    <?php if (empty($allAttributes)): ?>
                        <div class="text-muted">Aucun attribut global disponible.</div>
                    <?php else: ?>
                        <?php foreach ($allAttributes as $attribute): ?>
                            <?php
                            $attributeId = (int)$attribute['id'];
                            $selectedTermsForAttribute = $selectedAttributeTerms[$attributeId] ?? [];
                            $terms = $attributeTermsMap[$attributeId] ?? [];
                            ?>
                                ...
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                        ...
                                        <label style="display:flex; align-items:center; gap:0.5rem;">
                                            <input type="checkbox" name="attribute_ids[]" value="<?= $attributeId ?>" <?= in_array($attributeId, $selectedAttributeIds ?? [], true) ? 'checked' : '' ?>>
                                            <span>Utiliser cet attribut</span>
                                        </label>
                                    </div>
                                            ...
                                            <?php foreach ($terms as $term): ?>
                                                <div class="col-md-4">
                                                    <label style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.45rem;">
                                                        <input
                                                            type="checkbox"
                                                            name="attribute_term_ids[<?= $attributeId ?>][]"
                                                            value="<?= (int)$term['id'] ?>"
                                                            <?= in_array((int)$term['id'], $selectedTermsForAttribute, true) ? 'checked' : '' ?>
                                                        >
                                    ...
                                    <?php else: ?>
                                        <div class="text-muted mt-2">Aucun terme pour cet attribut.</div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
